      ...
      <div key={key}>
        <div className="flex items-center gap-3 rounded-lg border border-[var(--border)] bg-[var(--bg-card)] px-4 py-3">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {allowDelete && (
              <ConfirmBubble message={t('providers.confirmDelete')} detail={provider.label} onConfirm={() => handleDelete(provider)} disabled={!isEnabled || isDefault || loading[key]}>
                <button
                  disabled={!isEnabled || isDefault || loading[key]}
            ...
            {isEnabled && !isDefault ? (
              <ConfirmBubble message={t('providers.confirmDisable')} detail={provider.label} onConfirm={() => handleToggle(provider, false)} disabled={loading[key] || isDefault}>
                <Toggle
                  checked={isEnabled}
                  onChange={() => undefined}
