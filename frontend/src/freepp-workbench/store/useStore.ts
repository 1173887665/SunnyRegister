...
interface StoreState {
  ...
  selectedWorkspaceProjects: Set<WorkspaceProject>;
  toggleWorkspaceProject: (project: WorkspaceProject) => void;
...
export const useStore = create<StoreState>((set, get) => ({
  ...
  handleEvent: (evt) => {
    ...
    switch (evt.type) {
      ...
      // ---- 注册页账号操作进度 (REST 轮询 + WebSocket 双通道) ----
      case "reg_op_start": {
        const label = REGISTER_OPERATION_CN[evt.operation] || evt.operation || "账号操作";
        s.pushLog(`${label}开始${evt.total != null ? ` · ${evt.total} 项` : ""}${evt.message ? `：${evt.message}` : ""}`, "info");
        break;
      }
      case "reg_op_progress": {
        const label = REGISTER_OPERATION_CN[evt.operation] || evt.operation || "账号操作";
        const done = evt.done != null && evt.total != null ? ` ${evt.done}/${evt.total}` : "";
        const level = evt.ok === true || evt.status === "success" ? "ok" : evt.ok === false || evt.status === "failed" ? "err" : "info";
        s.pushLog(`${label}${done}${evt.message ? `：${evt.message}` : ""}`, level, evt.chain_id || "");
        break;
      }
      case "reg_op_complete": {
        const label = REGISTER_OPERATION_CN[evt.operation] || evt.operation || "账号操作";
        s.pushLog(evt.message || `${label}完成：成功 ${evt.success || 0}，失败 ${evt.failed || 0}`, evt.failed ? "warn" : "ok");
        break;
      }
      case "reg_op_error": {
        const label = REGISTER_OPERATION_CN[evt.operation] || evt.operation || "账号操作";
        s.pushLog(`${label}异常：${evt.message || evt.error || "未知错误"}`, "err");
        break;
      }
