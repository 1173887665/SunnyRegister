...
import { BRANCH_CN, BRANCH_NAMES } from "../types";
...
const BRANCH_TOKEN_SOURCE: Record<BranchName, string> = {
  paypal: "stripe",
  momo: "momo",
  grok: "grok",
  pix: "pix",
...
const ALL_CHAIN_SOURCES = Array.from(new Set(BRANCH_NAMES.map((branch) => BRANCH_TOKEN_SOURCE[branch])));
...
export function TokensView() {
  ...
  const [targetBranch, setTargetBranch] = useState<BranchName>(activeBranch);
  const [importScope, setImportScope] = useState<"all" | "current">("all");
  ...
  /** 当前视图标签: 注册账号可选择任意目标提链，其余来源跟随当前分支。 */
  const selectedBranch = BRANCH_NAMES.find((branch) => BRANCH_TOKEN_SOURCE[branch] === sourceFilter) || activeBranch;
  const viewBranchLabel = sourceFilter === "register"
    ? `注册账号 → ${BRANCH_CN[targetBranch] || targetBranch}`
    : BRANCH_CN[selectedBranch] || selectedBranch;
  const importSources = importScope === "all" ? ALL_CHAIN_SOURCES : [sourceFilter];
  const importTargetLabel = importScope === "all" ? `全部 ${BRANCH_NAMES.length} 个提链项目` : `${viewBranchLabel} 项目`;
  ...
  useEffect(() => {
    if (sourceFilter !== "register") setTargetBranch(activeBranch);
  }, [activeBranch, sourceFilter]);
  ...
  const handleProbe = async (t: Token) => {
    ...
    try {
      const r = await api(`/api/tokens/${t.id}/probe`, "POST", {
        branch: sourceFilter === "register" ? targetBranch : selectedBranch,
      });
  ...
  const handleBatchProbe = async () => {
    ...
    try {
      const r = await api("/api/tokens/probe", "POST", {
        ids,
        branch: sourceFilter === "register" ? targetBranch : selectedBranch,
      });
      if (r && r.ok) {
  ...
  const sessionBadge = (st: string | undefined) => {
    ...
    if (st.startsWith("error")) return { label: "探测失败", cls: "badge-danger" };
  ...
  const tokenErrBadge = (pe: string | undefined) => {
    ...
    return { label: "上游拒绝", cls: "badge-danger" };
  };
...
function collectTokens(o: any, out: CalibItem[]) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setResult(`已导入到${importTargetLabel}: 新增 ${r.imported ?? 0}, 去重/无效 ${r.failed ?? 0}, 库内共 ${tokensR?.tokens?.length ?? 0} 个`);
    } catch (e) {
  ...
  const handlePoolImport = async () => {
    ...
    try {
      ...
      if (r && r.ok) {
        setPoolResult(`拉取 ${r.total ?? 0}, 导入到${importTargetLabel}共 ${r.imported ?? 0}, 去重跳过 ${r.skipped ?? 0}`);
        const tokensR = await api("/api/tokens");
        if (tokensR && Array.isArray(tokensR.tokens)) {
          useStore.setState({ tokens: tokensR.tokens });
        }
  ...
  const runChain = async (ids: string[]) => {
    ...
    try {
      const res = await api("/api/chain/batch", "POST", {
        token_ids: ids,
        branch: sourceFilter === "register" ? targetBranch : activeBranch,
      });
  ...
  return (
    <div className="page page-wide">
        ...
        <div className="page-actions">
          <select
            ...
            onChange={(e) => {
              ...
              setSourceFilter(v);
              const branch = BRANCH_NAMES.find((item) => BRANCH_TOKEN_SOURCE[item] === v);
              if (branch) {
      ...
      <div className="card" style={{ marginBottom: 14 }}>
        <div className="card-head">
          <span className="card-title">导入 Token → {importTargetLabel}</span>
          <span className="card-hint">
        ...
        <div className="card-body tight" style={{ paddingBottom: 0 }}>
          <div className="inline-fields">
            <span className="muted" style={{ fontSize: 12 }}>导入范围</span>
            <select
              className="select flex-field-sm"
              value={importScope}
              onChange={(e) => setImportScope(e.target.value as "all" | "current")}
            >
              <option value="all">全部提链项目 ({BRANCH_NAMES.length})</option>
              <option value="current">仅当前项目 ({viewBranchLabel})</option>
            </select>
      ...
      <div className="card" style={{ marginBottom: 14 }}>
        <div className="card-head">
          <span className="card-title">从注册池导入 → {importTargetLabel}</span>
          <span className="card-hint">
        ...
        <table className="table">
          ...
          <tbody>
            ...
            {pageTokens.map((t) => {
              ...
              const probeDetail = String(probe.detail || probe.error || (t as any).session_type || "导入后自动探测 / 手动点击探测");
              return (
                <tr
                  key={t.id}
