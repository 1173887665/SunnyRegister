...
export interface ProxyNode {
  ...
  running: boolean;
  node_status?: string;
  auth_entry_ok?: boolean | null;
  auth_entry_status?: string;
  auth_entry_detail?: string;
  auth_entry_latency_ms?: number;
}
...
export interface RegEvent {
  ...
  type: "start" | "log" | "progress" | "complete" | "error" |
    "op_start" | "op_progress" | "op_complete" | "op_error";
  stage?: string;
  operation?: string;
  /** Account email attached to per-account registration/check events. */
  email?: string | null;
  message?: string;
  task_id?: string;
  total?: number;
...
export interface RegAccount {
  ...
  plan_status?: string;
  plan_detail?: string;
  /** Live accounts/check entitlement metadata */
  subscription_plan?: string;
  has_active_subscription?: boolean;
  is_active_subscription_gratis?: boolean;
  plus_trial_eligible?: boolean;
  plus_trial_campaign_id?: string;
  plus_trial_title?: string;
  plus_trial_summary?: string;
  plus_trial_discount_percentage?: number;
  plus_trial_duration_num_periods?: number;
  plus_trial_duration_period?: string;
  plus_trial_promotion_type_label?: string;
  eligible_offer_ids?: string[];
  plan_expires_at?: string;
  plan_renews_at?: string;
  plan_cancels_at?: string;
  billing_period?: string;
  billing_currency?: string;
  is_delinquent?: boolean;
  discount_type?: string;
  discount_amount?: number;
  discount_duration_num_periods?: number;
  discount_expires_at?: string;
  discount_cancellation_policy?: string;
  discount_promo_campaign_id?: string;
  last_purchase_origin_platform?: string;
  last_will_renew?: boolean;
  account_user_role?: string;
  plan_features_count?: number;
  can_access_with_session?: boolean;
  plan_http_status?: number;
  plan_error_code?: string;
  plan_last_success_at?: string;
  ...
  rebind_status?: "idle" | "queued" | "running" | "success" | "failed" | string;
  rebind_target_email?: string;
  rebind_error?: string;
  rebind_at?: string;
  rebind_token_status?: string;
  token_recovery_status?: "idle" | "running" | "success" | "failed" | string;
  token_recovery_error_code?: string;
  token_recovery_error?: string;
  token_recovery_at?: string;
  commerce_session_type?: string;
  commerce_promo?: string;
  commerce_payment_methods?: string[];
  commerce_probe_status?: "idle" | "success" | "failed" | "missing_token" | string;
  commerce_probe_error?: string;
  commerce_probed_at?: string;
}
...
export type MailboxProvider = "imap" | "mailcom_api" | "api798";
...
export interface Mailbox {
  ...
  auth_url?: string;
  auth_code?: string;
  mailcom_account?: string;
  mailcom_primary?: boolean;
...
export interface MailPoolData {
  ok: boolean;
  settings?: MailPoolSettings;
  rules: MailPoolRules;
  mailboxes: Mailbox[];
  presets: ImapPreset[];
}

export interface MailPoolSettings {
  enabled: boolean;
...
export type ViewName =
  | "overview" | "chains" | "logs" | "workspace"
...
export type WorkspaceProject =
  | "paypal_extract" | "momo" | "grok" | "pix" | "ideal" | "upi"
  | "kakao" | "blik" | "twint" | "bizum" | "gopay" | "naver_pay"
  | "gcash" | "grabpay" | "qris" | "direct" | "paypal" | "direct_pay";
...
export interface PipelineConfig {
  ...
  reg_country: string;        // 注册出口国家 (auto=随机; 指定国家码则优先对应代理节点)
