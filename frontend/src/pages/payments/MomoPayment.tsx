...
function MomoQrPayment({ accounts, jobs, settings, busy, run }: { accounts: Row[]; jobs: Row[]; settings: Row; busy: string; run: (key: string, action: () => Promise<any>, success: string) => Promise<void> }) {
  ...
  async function handleFile(event: ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0]; if (!file) return; setQrImage(URL.createObjectURL(file)); const Detector = (window as any).BarcodeDetector; if (!Detector) return; try { const bitmap = await createImageBitmap(file); const codes = await new Detector({ formats: ["qr_code"] }).detect(bitmap); if (codes[0]?.rawValue) setQrPayload(String(codes[0].rawValue)); bitmap.close(); } catch { /* Manual QR payload input remains available. */ } }
