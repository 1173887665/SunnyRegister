...
function RegisterView({ jobs, busy, run, refresh, onLogs }: { jobs: Row[]; busy: string; run: RunAction; refresh: () => Promise<void>; onLogs: (job: Row) => void }) {
  ...
  return <div className="gopay-view"><div className="gopay-section-title"><div><h2>注册与登录</h2><p>创建批量任务并处理短信验证码</p></div></div>
    <Panel title="新建批量任务"><form className="gopay-form" onSubmit={submit}><div className="gopay-form-grid">
      <label><span>号码来源</span><select name="source"><option value="pool">号码池</option><option value="smsbower">SMSBower</option></select></label>
...
function SettingsView({ sms, busy, run, refresh }: { sms: Row; busy: string; run: RunAction; refresh: () => Promise<void> }) {
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const data = new FormData(event.currentTarget); await run("sms", () => post("/sms-config", { api_key: data.get("api_key"), api_base_url: data.get("api_base_url"), service: data.get("service"), country: data.get("country") }), "SMSBower 配置已保存", refresh); }
  return <div className="gopay-view"><div className="gopay-section-title"><div><h2>系统配置</h2><p>GoPay 模块的 SMSBower 接口设置</p></div></div><Panel title="SMSBower 配置" className="gopay-settings-panel" action={<Status value={sms.api_key_configured ? "success" : "failed"} />}><form className="gopay-form" onSubmit={submit}><div className="gopay-form-grid"><label className="wide"><span>API Key</span><input name="api_key" type="password" autoComplete="off" placeholder={sms.api_key_configured ? `已配置 ${sms.api_key || ""}，留空保持不变` : "请输入 API Key"} /></label><label className="wide"><span>Base URL</span><input name="api_base_url" type="url" defaultValue={sms.api_base_url || "https://smsbower.page"} /></label><label><span>服务代码</span><input name="service" defaultValue={sms.service || "ni"} /></label><label><span>国家代码</span><input name="country" defaultValue={sms.country || "6"} /></label></div><Button type="submit" disabled={busy !== ""}><Settings2 className="mr-2 h-4 w-4" />保存配置</Button></form></Panel></div>;
}
