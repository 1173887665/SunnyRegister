...
function ActionMenu({
  ...
  return (
    <div className="relative flex min-w-[136px] items-center justify-end gap-1.5 whitespace-nowrap">
        ...
        <div className="relative">
          ...
          {open && typeof document !== 'undefined' && createPortal(
            ...
            >
              ...
              <ConfirmBubble message={`确认删除 ${acc.email}？`} onConfirm={async () => { setOpen(false); await apiFetch(`/accounts/${acc.id}`, { method: 'DELETE' }); onDelete() }} align="left">
                <button
      ...
      {actions.length === 0 && (
        <ConfirmBubble message={`确认删除 ${acc.email}？`} onConfirm={async () => { await apiFetch(`/accounts/${acc.id}`, { method: 'DELETE' }); onDelete() }}>
          <button
...
function Sub2ApiImportModal({
  ...
  const submit = async () => {
    ...
    try {
      ...
      const res = await apiFetch('/integrations/sub2api/import', { method: 'POST', body: JSON.stringify(payload) })
      setResult(res)
    } catch (e: any) {
      setError(e?.message || '导入 Sub2Api 失败')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="dialog-backdrop" onClick={onClose}>
      <div className="dialog-panel dialog-panel-sm max-w-2xl p-6" onClick={e => e.stopPropagation()}>
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <h2 className="text-base font-semibold text-[var(--text-primary)]">导入 Sub2Api 账号</h2>
            <p className="mt-1 text-xs text-[var(--text-muted)]">
              {hasSelection ? `将选中的 ${selectedIds.length} 个 ChatGPT 账号导入 Sub2Api 平台` : `将当前筛选范围内的 ChatGPT 账号导入 Sub2Api，共 ${total} 个账号`}
            </p>
          </div>
          <button onClick={onClose} className="rounded-full p-1 text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-primary)]">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <label className="text-xs text-[var(--text-muted)]">
            管理员 Token / Admin API Key
            <input
              value={form.admin_token}
              onChange={e => setForm(f => ({ ...f, admin_token: e.target.value }))}
              placeholder="x-api-key 或管理员 JWT"
              className="control-surface control-surface-mono mt-1"
              type="password"
            />
          </label>
          <label className="text-xs text-[var(--text-muted)]">
            认证 Header
            <select value={form.auth_header} onChange={e => setForm(f => ({ ...f, auth_header: e.target.value }))} className="control-surface mt-1 appearance-none">
              <option value="x-api-key">x-api-key（Admin API Key）</option>
              <option value="authorization">Authorization: Bearer（管理员 JWT）</option>
            </select>
          </label>
          <label className="text-xs text-[var(--text-muted)]">
            分组 ID
          ...
          <label className="text-xs text-[var(--text-muted)] sm:col-span-2">
            ...
            <input
              ...
              placeholder="OpenAI 默认分组"
              className="control-surface mt-1"
            />
          </label>
          <label className="text-xs text-[var(--text-muted)]">
            并发
            <input value={form.concurrency} onChange={e => setForm(f => ({ ...f, concurrency: e.target.value }))} className="control-surface mt-1" />
          </label>
          <label className="text-xs text-[var(--text-muted)]">
            优先级
            <input value={form.priority} onChange={e => setForm(f => ({ ...f, priority: e.target.value }))} className="control-surface mt-1" />
          </label>
          <label className="text-xs text-[var(--text-muted)]">
            倍率
            <input value={form.rate_multiplier} onChange={e => setForm(f => ({ ...f, rate_multiplier: e.target.value }))} className="control-surface mt-1" />
          </label>
          <label className="mt-6 inline-flex items-center gap-2 text-xs text-[var(--text-secondary)]">
            <input type="checkbox" checked={form.dry_run} onChange={e => setForm(f => ({ ...f, dry_run: e.target.checked }))} />
            仅测试，不实际导入 Sub2Api
          </label>
        </div>

        {error && <div className="mt-4 rounded-xl border border-red-500/20 bg-red-500/10 px-3 py-2 text-xs text-red-500">{error}</div>}
        {result && (
          <pre className="mt-4 max-h-64 overflow-auto rounded-xl border border-[var(--border)] bg-[var(--bg-hover)] p-3 text-xs text-[var(--text-secondary)]">
            {JSON.stringify(result, null, 2)}
          </pre>
        )}

        <div className="mt-5 flex gap-3">
          <Button onClick={submit} disabled={loading || total === 0} className="flex-1">
            {loading ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : <Download className="mr-1.5 h-3.5 w-3.5" />}
            {loading ? '导入中...' : form.dry_run ? '测试' : '导入'}
          </Button>
          <Button variant="outline" onClick={onClose} className="flex-1">取消</Button>
        </div>
      </div>
...
export default function Accounts() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-4 overflow-hidden">
      ...
      {showSub2ApiImport && <Sub2ApiImportModal platform={tab} total={total} statusFilter={filterStatus} searchFilter={debouncedSearch} selectedIds={[...selectedIds]} onClose={() => setShowSub2ApiImport(false)} />}
      ...
      <Card className="shrink-0 bg-[var(--bg-pane)]/40 border border-[var(--border)] shadow-sm">
        ...
        <div className="flex items-center justify-between gap-4 px-5 py-2.5 bg-[var(--bg-pane)]/20">
          ...
          <div className="flex items-center gap-2">
            ...
            {selectedCount > 0 && (
              <ConfirmBubble
                message={t('accounts.deleteSelectedConfirm', { count: selectedCount })}
                onConfirm={async () => {
